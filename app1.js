...
io.on('connection', (socket) => {
    ...
    socket.on('join', async (payload) => {
        ...
        try {
            ...
            if (!roomStates[room].users) {
                roomStates[room].users = [];
            }
            ...
            if (Array.isArray(roomStates[room].users) && !roomStates[room].users.some(user => user.userName === userName)) {
    ...
    socket.on('struckNumber', async (payload) => {
        ...
        try {
            ...
            // Check if calledNumbers array exists
            if (!roomStates[room].calledNumbers || !Array.isArray(roomStates[room].calledNumbers)) {
                console.log("Called numbers array doesn't exist for room:", room);
                roomStates[room].calledNumbers = [];
                return;
            }
    ...
    socket.on('category', async (payload) => {
        ...
        try {
            ...
            // Get the category card from room state and verify it exists
            const categoryCard = roomStates[room].categoryCard;
            if (!categoryCard || !categoryCard.category) {
                console.log("Category card doesn't exist for room:", room);
                return;
            }
            
            // Check if the category exists and is not already claimed
            const categoryItem = categoryCard.category.find(item => item && item.category === scoreCategory.category);
    ...
    socket.on('disconnect', () => {
        ...
        if (socket.data && socket.data.room && socket.data.userName) {
            ...
            if (roomStates[room]) {
                // Check if users array exists before filtering
                if (roomStates[room].users && Array.isArray(roomStates[room].users)) {
                    ...
                    console.log(`Room ${room} exists but users array is undefined or not an array`);
                    // Initialize users array if it doesn't exist
                    roomStates[room].users = [];
                }
            } else {
                console.log(`Room state for ${room} doesn't exist during disconnect`);
            }
        }
    });
